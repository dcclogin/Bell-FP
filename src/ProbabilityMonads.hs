...
--- | A distribution is represented as a list of possible values and their probabilities
...
uniform :: [a] -> Dist a
uniform xs = Dist . normP $ map (\x -> (x, 1.0)) xs
...
--- | Draw a single random outcome from a distribution according to its weights.
runDist :: Dist a -> IO a
runDist (Dist xs0) = do
    let xs = filter ((> 0) . snd) xs0
    case xs of
        [] -> error "runDist: empty distribution."
        _  -> do
            let total = sumP xs
            if total <= 0
                then error "runDist: total probability must be positive."
                else do
                    let cumulative = scanl1 (\(_, accP) (x, p) -> (x, accP + p)) xs
                    r <- randomRIO (0.0, total)
                    let pick [] = fst (last cumulative)
                        pick ((x, accP) : rest)
                            | r <= accP = x
                            | otherwise = pick rest
                    pure (pick cumulative)


--- | Draw a single random outcome from a normalized distribution.
runDistNormalized :: (Ord a) => Dist a -> IO a
runDistNormalized d = do
    let xs = unpackDist (squishD d)
    case xs of
        [] -> error "runDistNormalized: empty distribution."
        _  -> do
            r <- randomRIO (0.0, 1.0)
            let cumulative = scanl1 (\(_, accP) (x, p) -> (x, accP + p)) xs
                pick [] = fst (last cumulative)
                pick ((x, accP) : rest)
                    | r <= accP = x
                    | otherwise = pick rest
            pure (pick cumulative)
...
prodDist :: Dist a -> Dist b -> Dist (a, b)
prodDist da db = (,) <$> da <*> db

prodDist2 :: (Dist a, Dist b) -> Dist (a, b)
prodDist2 (da, db) = prodDist da db

prodDist3 :: (Dist a, Dist b, Dist c) -> Dist (a, b, c)
prodDist3 (da, db, dc) = (,,) <$> da <*> db <*> dc


type CondDist a b = a -> Dist b
type JointDist a b = Dist (a, b)


--- | general formula : p(a, b) = p(a) * p(b|a)
jointFromCond :: Dist a -> (a -> Dist b) -> Dist (a, b)
jointFromCond dA dB_paramA = do
    a <- dA
    b <- dB_paramA a
    return (a, b)

--- | general formula : p(b|a) = p(a, b) / p(a)
condFromJoint :: (Eq a) => Dist (a, b) -> (a -> Dist b)
condFromJoint dAB a = condD (\(a', _) -> a == a') dAB >>= \(_, b) -> pure b
